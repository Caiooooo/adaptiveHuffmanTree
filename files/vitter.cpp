...
{
	if ((*tree)->weight == weight && (*tree)->number > *number && (*tree)->parent->number != parent_number)
	{
...
void update(node **tree, unsigned char symbol, unsigned char *dictionary, node **nyt)
{
	...
	while (temp->parent != NULL)
	{
		...
		if (temp->parent != NULL)
		{
			...
			if (inner_temp != NULL)
			{
				// std::cout << inner_temp->number << std::endl;
				switch_node(temp->parent, l_r, inner_temp, l_r_sibling);
			}
		}
...
void get_the_code(node **tree, unsigned char symbol, char *do_code, std::queue<char> *code_write)
{
	...
	if ((*tree)->symbol == symbol && (*tree)->left == NULL&& (*tree)->left == NULL && (*tree)->right == NULL && (*tree)->weight != 0)
	{
...
void printTree(node **tree) // for debugging
{
	std::cout << "\n\n";
	...
	while (!q.empty())
	{
		...
		node *crrNode = q.front().first;
		if (crrNode->right != nullptr)
			q.push({crrNode->right, q.front().second + 1});
		if (crrNode->left != nullptr)
			q.push({crrNode->left, q.front().second + 1});
		q.pop();
		if (crrNode->symbol)
			std::cout << "|" << crrNode->symbol << ' ';
		else
			std::cout << "|" << crrNode->number << " ";
	}
}
